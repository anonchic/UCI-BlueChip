...
void leon23_wait_for(int ms) {
#ifdef CONFIG_LEON_3
    unsigned int old = LEON_BYPASS_LOAD_PA(&LEON3_GpTimer_Regs ->e[0].val);
#else
    unsigned int old = LEON_REGLOAD_PA(LEON_TCNT0);
#endif
    
    while(ms > 0) {
#ifdef CONFIG_LEON_3
	unsigned int new = LEON_BYPASS_LOAD_PA(&LEON3_GpTimer_Regs ->e[0].val);
#else
	unsigned int new = LEON_REGLOAD_PA(LEON_TCNT0);
#endif
	if (old > new) {
	    ms -= old - new;
	} else if (old < new){
	    ...
	    ms -= old;
#ifdef CONFIG_LEON_3
	    ms -= LEON_BYPASS_LOAD_PA(&LEON3_GpTimer_Regs ->e[0].rld) - new;
#else
	    ms -= LEON_REGLOAD_PA(LEON_TRLD0) - new;
#endif
	}
	old = new;
    }
...
static inline unsigned long do_gettimeoffset(void)
{
	...
	if (sparc_cpu_model == sparc_leon)
#ifdef CONFIG_LEON_3
		return ((LEON_BYPASS_LOAD_PA(&LEON3_GpTimer_Regs ->e[0].rld)&LEON_TCNT0_MASK) - 
			(LEON_BYPASS_LOAD_PA(&LEON3_GpTimer_Regs ->e[0].val)&LEON_TCNT0_MASK)) /*>> 2*/;
#else
