...
short leon_video_mem[LEON_VGAMEM_SIZE];
...
void __init setup_arch(char **cmdline_p)
{
	...
	{
		char b[1024];
		sprintf(b, "SRMMU_PTRS_PER_PMD: %d\nPAGE_SIZE: 0x%08x\nPTRS_PER_PTE: %d\n", SRMMU_PTRS_PER_PMD, PAGE_SIZE, PTRS_PER_PTE);
		console_print_LEON(b);
		sprintf(b, "SRMMU_PTRS_PER_PGD: %d\nSRMMU_REAL_PTRS_PER_PMD: %d\nSRMMU_REAL_PTRS_PER_PTE: %d\n", SRMMU_PTRS_PER_PGD, SRMMU_REAL_PTRS_PER_PMD, SRMMU_REAL_PTRS_PER_PTE);
		console_print_LEON(b);
		sprintf(b, "PTRS_PER_PTE/SRMMU_REAL_PTRS_PER_PTE:%d\n", PTRS_PER_PTE/SRMMU_REAL_PTRS_PER_PTE);
		console_print_LEON(b);
		sprintf(b, "[%d:%d(%d):%d]\n", LEON_PGD_SH, LEON_PMD_SH, LEON_PMD_SH_V, LEON_PTE_SH);
		console_print_LEON(b);
	...
	*cmdline_p = prom_getbootargs();
...
#if defined(CONFIG_VT) && defined(CONFIG_VGA_CONSOLE)
	{
	  ...
	  printk("Attaching textvga drivers [0x%x s:0x%x]: ",(unsigned int)&leon_video_mem,sizeof(leon_video_mem));
	  j = amba_get_free_apbslv_devices (VENDOR_GAISLER, GAISLER_VGA, dev, 1);  
	  if (j >= 1) {
	    leon_apbvga = (LEON3_VGA_Regs_Map *)dev[0].start;
	    printk("0x%x\n",(unsigned int)leon_apbvga);
	    conswitchp = &vga_con;
	  } else {
