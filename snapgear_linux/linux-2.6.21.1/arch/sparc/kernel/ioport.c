...
/*
 * For other architectures dma_* are wrappers around
 * the pci_ variants. With Leon we want to be able to
 * use them without PCI support.
 */
...
dma_alloc_coherent(struct device *dev, size_t size, dma_addr_t *dma_handle,
		   gfp_t flag)
{
	unsigned long len_total = (size + PAGE_SIZE-1) & PAGE_MASK;
	unsigned long va;
	struct resource *res;
	int order;
	...
	if (va == 0) {
		printk("dma_alloc_coherent: no %ld pages\n", len_total>>PAGE_SHIFT);
		return NULL;
	}
  
	if ((res = kmalloc(sizeof(struct resource), GFP_KERNEL)) == NULL) {
		free_pages(va, order);
		printk("dma_alloc_coherent: no core\n");
		return NULL;
	}
	...
	if (allocate_resource(&_sparc_dvma, res, len_total,
		...
		printk("dma_alloc_coherent: cannot occupy 0x%lx", len_total);
		free_pages(va, order);
		kfree(res);
		return NULL;
	}
	...
	*dma_handle = virt_to_phys(va); /* equals virt_to_bus (R.I.P.) for us. */
		    ...
		    dma_addr_t dma_handle)
{
	...
	if ((res = _sparc_find_resource(&_sparc_dvma,
		...
		printk("dma_free_coherent: cannot free %p\n", cpu_addr);
		return;
	}
	
	if (((unsigned long)cpu_addr & (PAGE_SIZE-1)) != 0) {
		printk("dma_free_coherent: unaligned va %p\n", cpu_addr);
		return;
	}
	
	size = (size + PAGE_SIZE-1) & PAGE_MASK;
	if ((res->end-res->start)+1 != size) {
		printk("dma_free_coherent: region 0x%lx asked 0x%lx\n",
		       (long)((res->end-res->start)+1), (long)size);
		return;
	}
	
	pgp = (unsigned long) phys_to_virt(dma_handle);	/* bus_to_virt actually */
	mmu_inval_dma_area(pgp, size);
	sparc_unmapiorange((unsigned long)cpu_addr, size);
	
	release_resource(res);
	kfree(res);
	
	free_pages(pgp, get_order(size));
}
		 ...
		 enum dma_data_direction direction)
{
	...
	if (direction != DMA_TO_DEVICE) {
		mmu_inval_dma_area((unsigned long)phys_to_virt(dma_addr),
			...
			enum dma_data_direction direction)
{
	BUG_ON(direction == DMA_NONE);
	if (direction != DMA_TO_DEVICE) {
		mmu_inval_dma_area((unsigned long)phys_to_virt(dma_handle),
				   (size + PAGE_SIZE-1) & PAGE_MASK);
	}
}
