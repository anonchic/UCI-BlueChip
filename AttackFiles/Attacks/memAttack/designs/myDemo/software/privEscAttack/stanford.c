...
extern __inline__ void trigger(void)
{
...
extern __inline__ void untrigger(void)
{
    __asm__ __volatile__(
       "xor %g0, 0, %g0\n\t"
	"or %g0, 0, %g0\n\t"
...
extern __inline__ void set_psr(unsigned int new_psr)
{
    __asm__ __volatile__(
        "wr %0, 0x0, %%psr\n\t"
        "nop\n\t"
...
/* rewrite (output); */
    ...
    set_psr(get_psr() ^ 0x80);
    
    printf("Attempting to do some bad stuff\n");
    trigger();
    printf("Switching back to supervisor mode\n");
    
    set_psr(get_psr() ^ 0x80);
    
    printf("Allowed to do bad things\n");
